  ...
  <div class="container mx-auto px-4 py-16 flex">
    ...
    <div class="ml-20">
      ...
      <div class="flex">
        ...
        <span class="ml-2 mt-2 text-gray-400 text-sm">{{ this.actor.birthday }} | {{ calculateAge }} years old </span>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
        <div :key="index" v-for="movie,index in this.knownFor">

          <router-link class="text-gray-300 text-sm leading-normal hover:text-white" :to="`/movie/${movie.id}`">
            <img :src="posterPath(movie)" alt="poster"
  ...
  <div class="credits border-b border-gray-800">
      <div class="container mx-auto px-4 py-16">
        ...
        <ul class="list-disc leading-loose pl-5 mt-8">
          <li :key="index" v-for="cast,index in castMovies">
...
export default {
  ...
  mounted() {
    ...
    this.fetchKnownForMovies(this.$route.params.id);
  ...
  computed: {
    ...
    calculateAge() {
      ...
      let actorYearOfBirth
      let actorMonthOfBirth
      let actorDayOfBirth
      if(this.actor.birthday){
        actorYearOfBirth = parseInt(this.actor.birthday.substring(0, 4));
        actorMonthOfBirth= parseInt(this.actor.birthday.substring(5, 7));
        actorDayOfBirth= parseInt(this.actor.birthday.substring(8, 10))
      }
      let age = today.getFullYear() - actorYearOfBirth;
      if (today.getMonth() < actorMonthOfBirth || (today.getMonth() == actorMonthOfBirth && today.getDate() < actorDayOfBirth)) {
        age--;
      }
      return age;
    }
  ...
  methods: {
    ...
    async fetchKnownForMovies(actorId) {
      const response = await this.$http.get("/person/" + actorId + "/combined_credits");
